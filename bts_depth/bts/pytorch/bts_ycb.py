...
def main_worker(args, parser):
    ...
    dataloader_eval = torch.utils.data.DataLoader(ds, batch_size=args.batch_size, shuffle=False)
    ...
    while epoch < args.num_epochs:
        for step, sample_batched in enumerate(dataloader):
            img, depth_gt, label, calibrate_params, folder = sample_batched
            img = img.cuda()
            ...
            lpg8x8, lpg4x4, lpg2x2, reduc1x1, depth_est = model(img, focal)
            
            # rescale preds and target to have same unit
            depth_est /= 10
            depth_gt /= 10000
